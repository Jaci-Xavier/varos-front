...
const App: React.FC = () => {
  ...
  const [contentDescription, setContentDescription] = useState<string>(`${allItems[0].description}`);
  ...
  return (
    <div className='w-[98vw]  flex flex-row justify-around'>
      <div className='mt-[70px] ml-[60px] w-[274px] h-[590px]'>
        ...
        >
          {carteiras.length > 0 && (
            <Menu.SubMenu key="sub1" title='Carteiras' className='text-[18px] font-bold leading-[24px] tracking-wider'>
              {carteiras.map((item) => (
                <Menu.Item key={`${item.key}`} className={`flex justify-around ${current === `${item.key}` ? 'bg-[#222729]' : ''}`} 
                  style={{ backgroundColor: current === `${item.key}` ? '#222729' : 'transparent' }}>
          ...
          {cursos.length > 0 && (
            <Menu.SubMenu key="sub2" title='Cursos' className='text-[18px] font-bold leading-[24px] tracking-widest text-left'>
              {cursos.map((item) => (
                <Menu.Item key={`${item.key}`} className={`flex justify-around ${current === `${item.key}` ? 'bg-[#222729]' : ''}`} 
                  style={{ backgroundColor: current === `${item.key}` ? '#222729' : 'transparent' }}>
                  <div className='flex flex-row items-center'>
          ...
          {constultoria.length > 0 && (
            <Menu.SubMenu key="sub3" title='Consultoria' className='text-[18px] font-bold leading-[24px] tracking-widest text-left'>
      ...
      <div id='72' className='w-[662px] h-[590px] mt-[60px] flex flex-col'>
        ...
        <img src={contentPhoto} alt={contentTitle} className='w-[621px] h-[329px] mt-2 ml-4 rounded-tl-[32px] rounded-tr-[32px] rounded-br-[32px]' />
      </div>
    </div>
  );
};
